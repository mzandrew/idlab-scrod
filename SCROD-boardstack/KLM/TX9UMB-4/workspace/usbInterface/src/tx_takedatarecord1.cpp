...
//			control->registerWriteReadback(board_id, 51, 0x200, regValReadback); //enable ASICs for readout
	...
	while(nEvt<numEvents){
		...
		//do software trigger
			...
			if(trigType == 1){
//				control->registerWrite(board_id,39,1<<15 | 1<<14 | 0b1111111111,regValReadback);//setting for using only the trig decision logic. trig based on X and Y logic, and keep all ASICs on

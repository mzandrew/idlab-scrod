...
int main(int argc, char* argv[]){
	...
	//Initialize
	cout<<"\nTesting RAM: writing a pattern and then reading it from RAM...\n";
	//int addr=1234;
	//int Wdata=28413,

	//write all

	for (int j=0;j<RAMn;j++)
	{
	int Rdata=0,Rdata1=0;

	addr=j;
	Wdata=(addr  & 0x00FF) ^ (addr>>8  & 0x00FF) ^ (addr>>16  & 0x00FF);
...
*/
	if (j%100 ==0)
		printf("Write Address= (%d,%x), W data= (%d,%x)\n",addr,addr,Wdata,Wdata);
	...
	addr=j;
	Wdata=(addr  & 0x00FF) ^ (addr>>8  & 0x00FF) ^ (addr>>16  & 0x00FF);//supposed to be this
	...
	if (j%100 ==0)
			printf("Read Address= (%d,%x), R data= (%d,%x), W data= (%d,%x) \n",addr,addr,Rdata1,Rdata1,Wdata,Wdata);

	if (Wdata!=Rdata1)
		printf("Error! Address=(%d,%x), W data= (%d,%x), R data=(%d,%x)\n",addr,addr,Wdata,Wdata,Rdata1,Rdata1);

...
//			control->registerWriteReadback(board_id, 51, 0x200, regValReadback); //enable ASICs for readout
	...
	while(nEvt<numEvents){
		usleep(10000);
			...
			control->registerWrite(board_id,39,1<<15 | ASICmask,regValReadback);//setting for using only the trig decision logic. also mask the not needed ASICs so they wont trigger.

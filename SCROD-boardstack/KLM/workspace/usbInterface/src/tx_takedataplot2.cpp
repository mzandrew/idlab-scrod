...
int main(int argc, char* argv[]){
	if (argc != 7){
    		std::cout << "wrong number of arguments: usage \n./tx_takedataplot2 <num events> <trig type: 0= SW, 1= HW> <Win start> <Win offset> <ASIC no> <op mode>" << std::endl;
    		return 0;
  	}
	...
	int WinS = atoi(argv[3]);
		...
		int ASICmask = atoi(argv[5]);
		if( ASICmask < 0 || ASICmask > 1023 ){
			std::cout << "Invalid ASIC no-- should be between 0 and 1023, exiting" << std::endl;
			return 0;
		}
...
//			control->registerWriteReadback(board_id, 51, 0x200, regValReadback); //enable ASICs for readout
	...
	while(nEvt<numEvents){
		...
		//do software trigger
			...
			if(trigType == 1){
				control->registerWrite(board_id,39,1<<15 | ASICmask,regValReadback);//setting for using only the trig decision logic and mask not needed ASICs

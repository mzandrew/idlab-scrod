...
int irs3BControlClass::setSCRODEEPROM(unsigned short int board_id_to_write, unsigned short int board_revision_to_write){

	int regValOut = 0;
	std::cout << "Programming EEPROM " << std::hex << board_id_to_write << std::dec << std::endl;
	...
	for( int i = 0 ; i < 14 ; i++ ){
		//try writing register, return if write fails
		if(!registerWriteReadback(board_id, I2C_BUS_0_WR, command_data[i], regValReadback))
			return 0;
		usleep(50000); //5ms pause as I2C bus is slow
		//check for ack
		if(!registerRead(board_id, I2C_BUS_0_RD, regValReadback))
			return 0;
	}
	return 1;
}

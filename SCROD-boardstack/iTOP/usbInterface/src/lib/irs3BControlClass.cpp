...
int irs3BControlClass::getWaveformData(unsigned int wavedatabuf[], int &wavedataSize){
	...
	int dataSize = 0;
	readPacketFromUSBFifo( databuf, 8192, dataSize );
...
//set ASIC DACs appropriate for IRS3Bs on carriers Rev C
int irs3BControlClass::initializeAsicDACs_irs3B_carrierRevC(unsigned int board_id){
	...
	for(int i = 0 ; i < numASICs ; i++ ){
		...
		if( !registerWriteReadback(board_id, VADJN_base + i, 24265, regValReadback) )
	...
	for(int i = 0 ; i < numASICs ; i++ ){
		...
		if( !registerWriteReadback(board_id, RCO_TARGET_base + i, 10913, regValReadback) )
	...
	if( !registerWriteReadback(board_id, TIMING_REG, 0x0015, regValReadback) )
	...
	registerWriteReadback(board_id, TIMING_REG, 0x0095, regValReadback);
	registerWriteReadback(board_id, TIMING_REG, 0x0015, regValReadback);
	registerWriteReadback(board_id, TIMING_REG, 0x0115, regValReadback);
	registerWriteReadback(board_id, TIMING_REG, 0x0015, regValReadback);
